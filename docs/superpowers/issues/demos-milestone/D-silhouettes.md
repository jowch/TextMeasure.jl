...
- `rasterize(polygon::Vector{Point2{Float64}}, cell_size::Real) -> BitMatrix` — `cell_size > 0` is the width and height of one terminal cell in polygon-coordinate units. Returns a BitMatrix with `true` indicating cells inside the polygon (point-in-polygon test on each cell center). **Coordinate convention:** `raster[row, col]` indexes row-major; `row == 1` corresponds to the **top** of the polygon's bounding box (y-down — matches the block-top frame used by `layout()` and `shape_pack`'s `Placement.y`). `col == 1` is the **left** of the bounding box. `raster_chord_fn` (in #C) consumes this convention; `D` and `C` must agree.

## Acceptance

- Shape validity smoke tests (CCW orientation, simple polygons, no self-intersections).
- `voronoi_shatter(poly, pt; n=4)` returns 4 polygons whose union equals `poly` within numerical tolerance and whose pairwise intersections are zero-measure. **Tolerance:** `GeometryOps.area(symdiff(union(shards), poly)) / GeometryOps.area(poly) < 1e-6`. Pairwise intersection check: `GeometryOps.area(intersection(shards[i], shards[j])) / GeometryOps.area(poly) < 1e-6` for all `i ≠ j`. Function used: `GeometryOps.union` / `intersection` / `symdiff` / `area`.
...
- **Name-clash disambiguation:** `AsteroidShapeModels.jl` exists on JuliaRegistries (under Astroshaper) but is unrelated — it loads 3D OBJ meshes for thermophysical asteroid simulation. We are NOT consuming or wrapping it; `asteroid_polygon` is a procedural 2-D toy.
